...
Vector3 Image::PorcentajeColor(Vector2 r, Vector2 g, Vector2 b, Vector2 p) {
	float maxR, maxG, maxB;

	Vector2 baricentro = (r * 0.333 + g * 0.333 + b * 0.334);

	float distanceR = distance(r, p);
	float distanceG = distance(g, p);
	float distanceB = distance(b, p);
	float distanceBar = distance(baricentro, p);

	if (distance(r, g) > distance(r, b)) {
		maxR = distance(r, g);
	}
	else {
		maxR = distance(r, b);
	}
	if (distance(b, r) > distance(b, g)) {
		maxB = distance(b, r);
	}
	else {
		maxB = distance(b, g);
	}
	if (distance(g, r) > distance(g, b)) {
		maxG = distance(r, g);
	}
	else {
		maxG = distance(g, b);
	...
	float landa = (maxR - distanceR) / maxR;
	float beta = (maxB - distanceB) / maxB;
	float gamma = (maxG - distanceG) / maxG;

	Vector3 ret(landa, beta, gamma);
	return ret;
}
